...
// Formata uma data valida de acordo com o parametro de entrada (metodo privado)
//	retorna o proprio valor de entrada se value nao for um objeto/data
const _formatDateToString = (value, formatStyle = 'dd/MM/yyyy HH:mm:ss') => {
...
// Tenta formatar um valor em uma data valida de acordo com o parametro de entrada (metodo privado)
//	retorna o proprio valor de entrada se data invalida
const _formatStringToDate = (value, formatStyle = 'dd/MM/yyyy HH:mm:ss') => {
			...
			if (typeof cValue === 'object' && cValue !== null) {
				if (!Object.prototype.hasOwnProperty.call(nDocument, nKey)) {
					if (cValue instanceof Date) {
						nDocument[nKey] = _formatDateToString(cValue);
					} else {
						nDocument[nKey] = {};
					}
				}
...
const setSort = (req, jsonData, toCamelCase = false) => {
	const _executeSort = (sortElements, sortOrder, sortCaseInsensitive) => {
		const sortThis = (a, b, i, iLen) => {
			...
			if (i < iLen) {
				...
				const aCheck = _formatStringToDate(getNestedValue(a, sortElements[i]) || '');
				const bCheck = _formatStringToDate(getNestedValue(b, sortElements[i]) || '');
	...
	if (req.method.toUpperCase() !== 'GET') {
		_throwThis('PAGINATOR (SORTER)', 400, 'Please use GET verb to perform the sort...');
	}
...
const setPage = (req, jsonDataAll, jsonData, jsonDataLen, toCamelCase = false) => {
	...
	if (req.method.toUpperCase() !== 'GET') {
		_throwThis('PAGINATOR (PAGE)', 400, 'Please use GET verb to perform the query...');
	}
